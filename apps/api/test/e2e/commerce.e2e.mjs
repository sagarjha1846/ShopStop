...
const offer = (await j('POST', `/threads/${thread.id}/messages`, { token: buyer, body: { kind: 'OFFER', offerMinor: 5500000 } })).data;
...
ok('seller accepts offer', accepted.status === 201 && accepted.data.amountMinor === 5500000);
...
ok('order PENDING at offer price', order1.status === 201 && order1.data.status === 'PENDING' && order1.data.totalMinor === 5500000, `total=${order1.data?.totalMinor}`);
...
const evt = { event: 'payment.captured', payload: { payment: { entity: { id: `pay_${RUN}`, order_id: providerOrderId, amount: 5500000, method: 'upi' } } } };
